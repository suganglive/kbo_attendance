...
# KBO
import kbo2022all
import kbo_2022_html
from holi import dfholi

### html file 불러오기 ####
# livesport data
html = kbo_2022_html.html
...
df_filtered = df[(df.date >= 20220401) & (df.date <= 20221011)]
df_filtered.index = range(720)
df = df_filtered
...
for idx, i in enumerate(df.values):
    ...
    # df[column] = df.groupby('date')[column].transform(lambda x: x.iloc[-1])
...
# KBO data
df2022 = pd.read_html(kbo2022all.html)[0]
# df2022 = df2022[:-1]
df2022 = df2022.drop(df2022.index[-1])
# print(df2022)
lst_temp = []
for i in df2022["날짜"]:
    i = i[0:4] + i[5:7] + i[8:]
...
merged_df.to_csv("./data/temp2022_1.csv")
# print(merged_df)
